...
def InheritGenes(gen, AllelesMutated, offspringno, offspring, genes, loci, muterate, mtdna, mutationans,
                 geneswap, epireset, Track_EpigeneReset1, Track_EpigeneReset2, startEpigene, epigeneans,
                 cdevolveans, noalleles, hindex):
    '''
    InheritGenes()
    Pass along genetic information to survived offspring from parents
    ...
    if epigeneans != 'N':
        if cdevolveans.split('_')[0] == 'M':
            ...
            selloci = 0
            epiloci = int(epigeneans.split('_')[1].split('L')[1])
        epiloci_index = list(range(selloci * int(cdevolveans.split('_')[3].split('A')[1]),
                                   selloci * int(cdevolveans.split('_')[3].split('A')[1]) + epiloci * int(
                                       epigeneans.split('_')[2].split('A')[1])))  # Index location for epi region, epi region only considers 2 alleles but generic form to index to here.
    else:
        epiloci_index = [-9999]
    ...
    if gen >= geneswap:
        ...
        if int(offspringno) != int(0):
            ...
            for i in range(offspringno):
                args = (i, gen, genes, offspring, loci, muterate, mtdna, mutationans, epigeneans, cdevolveans,
                        noalleles, hindex, epiloci_index, epireset, Track_EpigeneReset1, Track_EpigeneReset2,
                        countTwins)
            ...
            # Collect results
            offspring_updated = []
            alleles_mutated_total = 0
            for result in results:
                offspring_i, alleles_mutated_i = result
                offspring_updated.append(offspring_i)
                alleles_mutated_total += alleles_mutated_i

            # Update offspring
            offspring = offspring_updated

            # Now store the total number of alleles that mutated
            AllelesMutated.append(alleles_mutated_total)
